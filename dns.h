...
/**
 * \brief build a DNS response to a query
 *
 * \param *dns  the pointer to the response structure
 * \param *buf  the pointer to the received query
 *
 * \return Returns the size of DNS
 */

int build_dns_answer(uint8_t *dns, uint8_t *buf);

/**
 * \brief parse the query of a DNS request
 *
 * \param *answer   the pointer to the answer structure to fill up
 * \param *host     Host for loockup
 * \param *file     Name of the file to parse for DNS records
 * \param *num      Number of answer structures filled up
 * \param *offset   Offset to the initial structure
 * \param type
 * \param class
 */

void get_dns_record(char* host, uint8_t *answer, char* file, int* num, int* offseet, int type, int class);
